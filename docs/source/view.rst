...
and processing spaces (cf. :ref:`structure`) and yields multiples plots for data exploration, features analysis, features discussion, 
...
Plot Categorical Features Comparison :meth:`~watex.view.ExPlot.plotcatcomparison` 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`~watex.view.ExPlot.plotcatcomparison` compares two ordinal categories. It simulates that the the bining of `xname` into a `q` 
...
.. code-block:: python 
	...
	>>> data = fetch_data ('bagoue original').get('data=dfy1') 
	>>> p= ExPlot(tname='flow', fig_size =(7, 5) ).fit(data)
	>>> p.plotcatcomparison(xname ='sfi', yname='ohmS') # compare 'sfi' and 'ohmS'
...
:meth:`~watex.view.QuickPlot.corrmatrix`  method quickly plots the numerical and categorical features by setting the parameter `cortype` 
to ``num`` or ``cat`` for numerical or categorical features. Here is an example using the `Bagoue dataset` with mixture type of features:
...
:meth:`~watex.view.TPlot.plot_ctensor2d` plot the filtered tensor apply applying the filtered function. Refer to :ref:`methods` to 
