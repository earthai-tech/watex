...
class _sensitive:
    ...
    def which_file(cls, filename=None, deep= True): 
        ...
        *logfile*       Occam Logfile, Get after inverson (Inversion file )
        ...
        if _filename is None : 
            WATexError_file_handling('Error file: NoneType can '
        ...
        if _flag !=0 :  
            ...
            if _flag =='edi': #avoid expensive function then controle 2thing before continue 
                if _value_code [0] not in  _data_lines[0] :
                        ...
                        raise WATexError_file_handling(
            ...
            if _ccounter <7 :
                raise WATexError_file_handling('ERROR occurs while reading file !<{0}>. '
            ...
            try :
                ...
                assert (len(resp0) == len(resp_end)) and (len(resp_end)==len(resp_mid)),\
                    WATexError_file_handling('We presume that  RESP <{0}> file doesnt not match a OCCAM-RESPONSE file.'.format(os.path.basename(_filename)))
            except :
    ...
    def validate_avg(avg_data_lines):
        ...
        if sum(num) ==0 : raise WATexError_file_handling(
