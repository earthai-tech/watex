...
# originallly created on Tue Dec 29 19:18:44 2020
...
def get_stationid (stations ,  station_id) :  
    ...
    for iid in station_id : 
        ...
        if isinstance(iid, (int, float)): 
            ...
            #the stations numberred start from 1 to len(stations)
...
def get_frequency_id (freq_array , frequency_id ): 
    ...
    for freqid in frequency_id:
        if isinstance(freqid ,str) :
            ...
            except : raise Wex.WATexError_plot_tip(
                    'Frequency value must be an integer of float not str ' )
        if freq_array.min() > freqid > freq_array.max() : 
            warnings.warn (
                'Can not find the id <{0}>. ID provided is out of'
                ' the range.Frequency trend to: <{1}|{2}Hz to {3}|{4}Hz>.'.\
                format(freqid, np.where(freq_array==freq_array.min()),
                       freq_array.min(),
...
def slice_matrix (base_matrix , freq_array, doi=2000):
    ...
    if isinstance (doi, str):
        try : doi =float(doi)
            ...
            try : doi =float(ndoi)
                ...
                warnings.warn (
                    'Can not convert <{0}> into float number. '
                     'Depth of investigation value must be'
                     ' float number.'.format(doi))
                raise Wex.WATexError_plot_tip(
    ...
    # take the first lien that value is greater than 
...
def delineate_curve ( dict_loc , value , atol=0.2, replace_value = np.nan): 
    ...
    if isinstance(value, (str, int, float)): 
        try : value =float(value)
            ...
            warnings.warn(
                'Delineation value is float number not <{0}>'.format(
                    type(value)))
            raise Wex.WATexError_plot_tip(
        ...
        try : 
            ...
            warnings.warn(
                'Delineation value is float number not'
                ' <{0}>'.format(type(value)))
            raise Wex.WATexError_plot_tip(
                'Input delineation value is unacceptable. '
                'Please enter a float number.')
    try : 
        ...
        if atol > 1  or atol <0: 
            warnings.warn (
                'Tolerance value is <=1 (less or egal than 1). '
                'Please enter again a new value.')
            raise Wex.WATexError_plot_tip(
                'Tolerance value is assumed to be 0<= '
                'atol <=1 .Please check your value.')
    except : 
        warnings.warn('Tolerance value <{0}> provided is wrong. '
                      'Must be a float number 0<= atol <=1  not '
                      'a <{1}>'.format(atol, type(value)))
        raise Wex.WATexError_plot_tip(
    ...
    for value_to_find  in value :
        ...
        for keys, value_in_dict in sorted(dict_loc.items()):
            if value_to_find in value_in_dict:  
                ...
                if tem==[] : 
                    # append None value outside the tolerance param.
...
def delineate_sparseMatrix (dict_loc, delineate_dict, replace_value =np.nan ):
    ...
    value doesnt exist in the delineate dict will 
    be repalce by replacevalue . Default is `nan`.
    ...
    :param replace_value: 
        value to replece other value  like build a sparse matrix 
...
def resetting_ticks ( get_xyticks,  number_of_ticks=None ): 
    ...
    if not(number_of_ticks, (float, int)): 
        try : number_of_ticks=int(number_of_ticks) 
            ...
            raise Wex.WATexError_plot_tip('<{0}> detected. Must be integer.')
...
def slice_csamt_matrix ( block_matrix , station_offsets, 
    ...
    if offset_min < offset_start : 
        warnings.warn (
            'Povided minimal offset is out of the range !'
            'Offset minimal for plottig is ={0}'.format(offset_start))
        raise Wex.WATexError_plot_tip(
            'Povided minimal offset out of the range !Offset'
            ' minimal for plottig is ={0}'.format(offset_start))
    if offset_max > offset_end  : 
        warnings.warn (
            'Povided maximal offset is out of the range !'
    ...
    if doi > depth_offsets.max() : 
        mess ='depth <{0}> is out of depth range. '.format(doi)+\
            'Maximum depth for is={0}.'.format(depth_offsets.max())
        warnings.warn(mess)
        raise Wex.WATexError_plot_tip(mess)
    ...
    if isinstance (doi, str):
        try : doi =float(doi)
            ...
            try : doi =float(ndoi)
                ...
                raise Wex.WATexError_plot_tip(
                    'Value provided <{0}> must '+\
                        'be float number not <{1}>'.format(doi, type(doi)))
            else :
                if fm==1 : doi *= 1e3   # convert into m
...
def build_new_station_id (station_id , new_station_name ): 
    """
    Fonction to build new station id including station name provided , 
    if the length provided doesnt match the length of station id 
    ...
    :Example:
        ...
        >>> print(sto)
        >>> stn, fu =punc.build_new_station_id(
        ...    station_id = sto, new_station_name =ts)
        >>> print(stn, fu)
    """
    mess=None
    ...
    else : # for consistenncy rebuild sites name and keep on str value 
        new_station_name=[str(stn) for stn in new_station_name] 
    if get_len_new_station_id <  len(station_id ): 
        mess ='Length of sites names provided ={0}. '\
            'It doesnt match the length of  station id ={1}.'\
...
def get_conductive_and_resistive_zone (
        data, site_names, purpose ='groundwater',**kws): 
    ...
    def fmt_text (data_text, fmt='~', leftspace = 3, return_to_line =77) : 
        ...
        :param leftspace: 
            How many space do you want before starting wrinting report .
        :type leftspae: int 
        ...
        for  ii, num in enumerate(data_text) : # loop the text 
            ...
            if ss == return_to_line :  # if enconter 50character then return chariot 
                if data_text[ii+1] !=' ' : 
                    ...
                    #if the word is brken then add "-" and return chariot 
                else : 
    ...
    if  imins < len(site_names) :
        print(
            '---> Probably very conductive zone is = {0} with rho = {1} Ω.m.'.\
        ...
        # model objet then bring model file 
            if model_offsets is None  or site_offsets is None: 
                mess =''.join([' !Could not find Occam 2D model '
                               'resistivity value . Could not provide a report.', 
                               'Too many minimum value find !'
                               ' Need Occam model resistivity data and model ', 
                               'station data to provide a report.'])
                warnings.warn(mess)
    ...
    if len(temcon) >0 : 
        for stn, rhoval  in temcon : 
            if len(temcon)>1 : # if many conductive zone has ben selected then : 
                print(
                    '--> {0} Probably  conductive zones has been selected :'.
                    format(len(temcon)))
    ...
    if len(temres) > 0: 
        for stn, rhoval in temres : 
            if len(temres)>1 : # if many conductive zone has ben selected then : 
                print(
                    '--> {0} Probably  resistives zones has been selected :'.
                    format(len(temres)))
            print('---> Probably very resistive zone is = {0} with rho = {1} Ω.m.'.\
                  format(stn, np.power(10,rhoval)))
    ...
    if purpose =='groundwater':
        ...
        if pseudo_trust >  np.sqrt(2)* p_ratio_max *100 : 
            ...
            fm='However,'
             ...
             mess ='hydrogeologically favorable zone and could be '\
                 'a right drilling point.' 
             fm=''
...
def find_local_maxima_minima (array): 
    ...
    if isinstance(array, (list, tuple)): # put on list 
        ...
        except : raise Wex.WATexError_parameter_number(error_mess)
            
    if isinstance(array, (float, str, int)):
        try :
            array =np.array([float(array)])
        except : Wex.WATexError_inputarguments(error_mess)
...
@deprecated("function deprecated it's does not cover the "
            "total depth when last value is outside the model depth range. ")
...
def average_rho_in_deep (dep_array, rho_array, step_descent)   :
    ...
    if isinstance(step_descent, (str, int)): 
        try : 
            ...
            raise Wex.WATexError_plot_geoinputargument(
    ...
    if step_descent < dep_array.min(): 
        raise Wex.WATexError_inputarguments(
            'Value provided ={0} m is less than the'
            ' minimum depth ={1} m.'.format(step_descent, dep_array.min()))
    
    if step_descent > dep_array.max(): 
        raise Wex.WATexError_inputarguments(
            'Value provided is = {0} m is greater than'
            ' maximum depth ={1}m.'.format(step_descent, dep_array.max()))
...
def get_station_id_input_resistivities(station_rho_value, number_of_layer=None): 
    ...
    if isinstance(number_of_layer, (list, tuple, np.ndarray)): 
        try : 
            ...
            raise Wex.WATexError_plot_tip(
                'Could not convert number of  layer ={0} '
                'value into integer.'.format(tuple(number_of_layer)))
            
    if isinstance(number_of_layer, (float, str)): 
        try : int(number_of_layer)
        except : Wex.WATexError_plot_tip(
                'Could not convert number of layer ={} to integer.'.format(
                    number_of_layer))
    ...
    if isinstance(station_rho_value, (list, tuple)): 
        ...
        except : raise Wex.WATexError_plot_geoinputargument(
    ...
    return np.linspace(station_rho_value.min(), 
                       station_rho_value.max(), number_of_layer)
...
def build_resistivity_barplot(depth_values , res_values): 
    ...
    mess = "".join(["Depth and resistivity arrays have no the same Length .",
                    " Length Depth is ={0} "
                    "and length depth is = {1}".format(len(depth_values), 
                                                       len(res_values))
                    ])
    if len(depth_values) !=len(res_values) : 
        warnings.warn(mess)
        raise Wex.WATexError_geodrill_inputarguments(
...
def annotate_tip(layer_thickness , layer_names): 
    ...
    if len(layer_thickness)==0 : 
        mess = 'None value is provided, Please specify at least the '\
            'thickness of one truth layer.'
        warnings.warn(mess)
        raise Wex.WATexError_plot_tip(mess)
