...
class ResPhase(object):
    ...
    def compute_resistivity_phase(self, 
        ...
        if self._z is None or self.freq is None:
            raise ZError('Values are None, check _z, _z_err, freq')
    ...
    def set_res_phase(self, res_array, phase_array, freq, res_err_array=None,
        ...
        if np.linalg.norm(np.imag(phase_array)) != 0:
            raise ValueError(
                'Error - array "phase" isnot real valued !')
        ...
        for idx_f in range(self.freq.shape[0]):
            for ii in range(2):
                for jj in range(2):
                    ...
                    self._z_err[idx_f, ii, jj] = max(propagate_error_polar2rect(
                        abs_z,
                        abs_z_error,
...
class Z(ResPhase):
    ...
    def remove_distortion(
        ...
        dummy, DI_err = invertmatrix_incl_errors(distortion_tensor,
...
class Tipper(object):
    ...
    def tipper(self, tipper_array):
        ...
        if self._tipper is not None and self._tipper.shape != tipper_array.shape:
            raise ZError('Shape of new "tipper" array does not match old' + \
                    ...
                    if self.tipper_err is not None:
                        if type(self.tipper) == np.ma.core.MaskedArray:
                            if self.tipper.mask[idx_f, 0, jj]:
                                continue
                        r_err, phi_err = propagate_error_rect2polar(
...
def correct4sensor_orientation(Z_prime, Bx=0, By=90, Ex=0, Ey=90,
    ...
    try:
        ...
        raise ValueError("ERROR - Given angles do not" + \
